...
/*
 @routes 
        ...
        2. registrationsbydate: filtering registrations by date send pageno query and dateBegin (format yyyy-mm-dd) and dateEnd (format yyyy-mm-dd) as query ::: example : http://localhost:3000/registrationsbydate?pageno=1&dateBegin=2019-11-19
        
        3. registrationsbycourse : filtering registrations by courses send pageno query and courseId as query
        
        4. registrationsbyinstitute :  filtering registrations by Institutes send pageno query and institute as query
*/
...
router.get("/registrationsbyinstitute", async(req, res) => {
    pageNo = req.query.pageno;
    pagesize = 15;
    reqInstitute = req.query.institute;
    registrations = await Registrations.find({ institute: reqInstitute })
        .skip(pagesize * (pageNo - 1))
        .limit(pagesize);
    res.send(registrations);
});
